...
public:
	...
	// vectorに追加
	void SetDrawItem(int x1, int y1, int graphicHandle, float priority, int DrawType = 0, double angle = 0.0f, int BlendMode = 0, int BlendParam = 0, int x2 = 0, int y2 = 0);
...
private:
	struct DrawInfo {
		...
		void SetInfo(int x1,int y1, int graphicHandle, float priority, int DrawType, double angle, int BlendMode, int BlendParam, int x2, int y2) {
			_x1 = x1;
			_y1 = y1;
			_x2 = x2;
			_y2 = y2;
