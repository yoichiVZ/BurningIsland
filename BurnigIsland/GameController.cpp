		...
		if (_island[i]->DistanseCheck(_player._posX, _player._posY)) {
			...
			for (int j = 0; j < ISLAND_NUM; j++) {
				if (_rope._connectDate[i][j] == 1 || _rope._connectDate[j][i] == 1 ||
					_rope._connectDate[num_p][j] == 1 || _rope._connectDate[j][num_p] == 1) // ”½‘Î‘¤‚©‚ç‚àƒ[ƒv‚ð‚©‚¯‚ç‚ê‚é
...
void GameController::Draw()
{
	...
	if (!_fireReloadFlag) {
		DrawFormatString(10, 30, GetColor(255, 255, 255), "Œ‚‚Ä‚é");
	}
